$(document).ready(function(){
	...
	var public_spreadsheet_url = 'https://docs.google.com/spreadsheets/d/1lxGmqmLxVtgG8oDiIVDmQepbizIHDuDlKEAWFsKsMdw/pubhtmlx';

    Tabletop.init( { key: public_spreadsheet_url, callback: displayDB, simpleSheet: true } );
	  
	$("#homeButton").click(function() {
		if ($("#homeSearchBar")[0].value.length !== 0) { // If user enters a string 
			// Store
			localStorage.setItem("searchText", ($("#homeSearchBar")[0].value));
			console.log("The searchText is:" + localStorage.getItem("searchText"));
			// Retrieve
			//document.getElementById("#searchBar").innerHTML = localStorage.getItem("searchText");
			for (j = 0; j < bookList.length; j++) {
				if (bookList[j].title === localStorage.getItem("searchText")) { // If movie title equals search bar
			...
			alert("Sorry, we don't have this movie. Please try again.")
